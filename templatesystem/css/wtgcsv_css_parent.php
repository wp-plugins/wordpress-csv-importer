...
if($side == 'admin'){
    ...
    */
    function wtgcsv_register_admin_styles() {
        ...
        wp_register_style(WTG_CSV_ABB . 'css_jquerymultiselect_asset',plugins_url(WTG_CSV_FOLDERNAME.'/templatesystem/script/multiselect/assets/style.css'), __FILE__);
    ...
    add_action('admin_print_styles', WTG_CSV_ABB . 'styles_callback');
    
    function wtgcsv_styles_callback() {
        // jQuery UI Styling
        wp_enqueue_style(WTG_CSV_ABB . 'jquery_styles');
        // notifications
        wp_enqueue_style(WTG_CSV_ABB . 'css_notification');
        // plugins own style sheets
        wp_enqueue_style(WTG_CSV_ABB . 'css_admin');
        wp_enqueue_style(WTG_CSV_ABB . 'css_global');

        // multiselect (checkbox menus)
        wp_enqueue_style(WTG_CSV_ABB . 'css_jquerymultiselect');
        wp_enqueue_style(WTG_CSV_ABB . 'css_jquerymultiselect_asset');
        wp_enqueue_style(WTG_CSV_ABB . 'css_jquerymultiselect_prettify');

        // multi-select (not the same as multiselect, these are lists)
        wp_enqueue_style(WTG_CSV_ABB . 'css_jquerymulti-select');

        // accordian
        wp_enqueue_style(WTG_CSV_ABB . 'css_jqueryaccordion');
        
        // file uploader @ file uploader @ http://pixelcone.com/fileuploader/demo/
        wp_enqueue_style(WTG_CSV_ABB . 'css_fileuploader');                     
    }
