...
function wtgcsv_admin_menu(){
   ...
   if(!$wtgcsv_is_installed){
        ...
        add_submenu_page($n, __($wtgcsv_mpt_arr['projects']['title'],$n.$wtgcsv_mpt_arr['projects']['slug']), __($wtgcsv_mpt_arr['projects']['menu'],$n.$wtgcsv_mpt_arr['projects']['slug']), $wtgcsv_mpt_arr['projects']['role'], $wtgcsv_mpt_arr['projects']['slug'], WTG_CSV_ABB . 'page_projects');
        add_submenu_page($n, __($wtgcsv_mpt_arr['creation']['title'],$n.$wtgcsv_mpt_arr['creation']['slug']), __($wtgcsv_mpt_arr['creation']['menu'],$n.$wtgcsv_mpt_arr['creation']['slug']), $wtgcsv_mpt_arr['creation']['role'], $wtgcsv_mpt_arr['creation']['slug'], WTG_CSV_ABB . 'page_creation');        
        ...
        add_submenu_page($n, __($wtgcsv_mpt_arr['more']['title'],$n.$wtgcsv_mpt_arr['more']['slug']), __($wtgcsv_mpt_arr['more']['menu'],$n.$wtgcsv_mpt_arr['more']['slug']), $wtgcsv_mpt_arr['more']['role'], $wtgcsv_mpt_arr['more']['slug'], WTG_CSV_ABB . 'page_more');
    }
}
...
function wtgcsv_header_page($pagetitle,$layout){
  ...
  <script language="javascript">
    ...
    <div class="wrap">
        ...
        <p>Please do not discuss or report faults online. Please do not rate the plugin on Wordpress.org until it is
        complete. Thank you for respecting our plugin.</p><h4>Known Bug</h4><p>Testers have reported the plugin interface
        to stop working properly in Google Chrome but not in Firefox or Explorer. The problem is JavaScript related. Work
        is being done on CSS only styling as an alternative for the sake of testing, while the problem is resolved and for
        future testing that requires JavaScript to be disabled.</p>','warning','Extra','ALPHA Wordpress CSV Importer'); ?>
    ...
    foreach($wtgcsv_easyquestions_array as $key => $question){
        ...
        if($question['active'] != true){
            ...
            if($question['type'] == 'single'){?>
                ...
                <p> 
                    <select id="'.WTG_CSV_ABB.'single'.$singles_created.'" title="Please click on a single option" multiple="multiple" name="example-basic" size="8">
                        '.$optionlist.'
                    </select>
                ...
                echo wtgcsv_notice($question['question'] . ' ' . wtgcsv_link('?',$question['helpurl'],'','_blank','','return','Click here to get more help for this question') .'
                <p> 
                    <select id="'.WTG_CSV_ABB.'multiple'.$multiple_created.'" title="You may select multiple options" multiple="multiple" name="example-basic" size="8">
                        '.$optionlist.'
                    </select>
                </p>','question','Small','','','return');?>
...
function wtgcsv_menu_csvfiles($range = 'all',$id = 'noid'){?>
    <p>
        <select id="wtgcsv_multiselect<?php echo $id;?>" name="<?php WTG_CSV_ABB . 'csvfiles_menu';?>">
...
*/
function wtgcsv_display_alldesigns_buttonlist(){
...
*/
function wtgcsv_display_projectdesigns_buttonlist($form_id){
...
function wtgcsv_add_custom_boxes_contenttemplate() {
    add_meta_box( 
        ...
        'Template Design Type',// TODO: translate
...
* Adds a box to the Content Design edit page. 
*/
function wtgcsv_add_custom_boxes_titletemplate() {
    add_meta_box( 
        'wtgcsv_custombox_titletemplatetype_id',// a unique id of the box being displayed on edit page
        'Template Design Type',// TODO: TRANSLATION
        'wtgcsv_custombox_titletemplatetype',// callback to function that displays html content insie box on page
        'wtgcsvtitle'// post type to display meta box on 
    );
}
...
function wtgcsv_custombox_contenttemplatetype( $post ) {
  ...
  echo 'Description for this field';
  echo '</label> ';
  echo '<input type="text" id="myplugin_new_field" name="myplugin_new_field" value="A value in text field" size="25" />';
}

/**
* Print html content for custom meta box within custom post type wtgcsvtitle
* 
* @todo HIGHPRIORITY, change the input name and id with proper testing
*/
function wtgcsv_custombox_titletemplatetype( $post ) {
 ...
 * 
 * @link http://jquery.aamirafridi.com/jst/ - scroll plugin (added March 2012)
 * @global $wtgcsv_mpt_arr, set in variables_admin.php either using a local stored array or coded array depending on state (see argument)
 * @param string $thepagekey (page id)
 * @param integer $opentab (use to open specific tab other than the first one)
 * 
 * @todo LOWPRIORITY, use event: "mouseover"  in script but with option to switch it off (only add this one all javascript has been improved)
 */
function wtgcsv_createmenu($thepagekey){
    $nav_type = 'jquery';
    if($nav_type == 'css'){
        wtgcsv_navigation_css($thepagekey);
    }elseif($nav_type == 'jquery'){
...
function wtgcsv_selectables_csvfiles($range = 'all',$id = 'noid'){?>
    <div id="wtgcsv_selectcsvfiles_<?php echo $id;?>">
        <select multiple='multiple' id="listtestidtest" name="wtgcsv_csvfilearray_<?php echo $id;?>[]">
            <?php wtgcsv_option_items_csvfiles('all');?>
        </select>
    </div>
...
function wtgcsv_display_designtype_menu($post_id){
    ...
    <p>        
        ...
        <?php $optionmenu .= '<select name="wtgcsv_designtype" id="wtgcsv_select_designtype" multiple="multiple">';?>
...
function wtgcsv_table_customfield_rules_basic(){
    ...
    if(!isset($project_array['custom_fields']['basic'])){
        wtgcsv_notice('You do not have any basic custom field rules for adding meta data too your posts.','info','Small');
    }else{    
...
function wtgcsv_display_posttyperules_byvalue_table(){
    ...
    if(!isset($project_array['posttyperules']['byvalue'])){
        ...
        foreach( $project_array['templaterules']['byvalue'] as $key => $rule ){
            echo '<tr class="first">
...
function wtgcsv_display_templatedesignrules_byvalue_table(){
    ...
    $project_array = wtgcsv_get_project_array($wtgcsv_currentproject_code);
    if(!isset($project_array['templaterules']['byvalue'])){
...
*/
function wtgcsv_display_databasetables_withjobnames($checkbox_column = false){
    global $wtgcsv_dataimportjobs_array,$wtgcsv_jobtable_array;
