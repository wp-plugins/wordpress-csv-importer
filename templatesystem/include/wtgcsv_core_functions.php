 ...
 */
function wtgcsv_notice($message,$type = 'success',$size = 'Extra',$title = false, $helpurl = 'http://www.wordpresscsvimporter.co.uk/support', $output_type = 'echo',$persistent = false,$clickable = false){
    if(is_admin()){
        ...
        if($helpurl != false && $helpurl != '' && $helpurl != 'http://www.wordpresscsvimporter.co.uk/support'){$clickable = false;}
            
        if($output_type == 'return'){
            return wtgcsv_notice_display($type,$helpurl,$size,$title,$message,$clickable,$persistent);
        }else{
            global $wtgcsv_notice_array;

            // establish next array key
            $next_key = 0;
            if(isset($wtgcsv_notice_array['notifications'])){
                $next_key = wtgcsv_get_array_nextkey($wtgcsv_notice_array['notifications']);
            }
            
            // add new message to the notifications array
            $wtgcsv_notice_array['notifications'][$next_key]['message'] = $message;
            $wtgcsv_notice_array['notifications'][$next_key]['type'] = $type;
            $wtgcsv_notice_array['notifications'][$next_key]['size'] = $size;
            $wtgcsv_notice_array['notifications'][$next_key]['title'] = $title;
            $wtgcsv_notice_array['notifications'][$next_key]['helpurl'] = $helpurl; 
            $wtgcsv_notice_array['notifications'][$next_key]['clickable'] = $clickable;
            $wtgcsv_notice_array['notifications'][$next_key]['persistent'] = $persistent;
...
*/
function wtgcsv_notice_display($type,$helpurl,$size,$title,$message,$clickable,$persistent = false){
...
function wtgcsv_notice_output(){
    global $wtgcsv_notice_array;
    if(isset($wtgcsv_notice_array['notifications'])){
        foreach($wtgcsv_notice_array['notifications'] as $key => $notice){
            echo wtgcsv_notice_display($notice['type'],$notice['helpurl'],$notice['size'],$notice['title'],$notice['message'],$notice['clickable'],$notice['persistent']);                                               
        }
    }  
}
...
* @return mixed
* 
* @link http://www.importcsv.eu/hacking/project-array-wtgcsv_project_array
*/
