    ...
    if(isset($_POST['wtgcsv_post_processing_required'])){
        if(isset( $_POST['wtgcsv_hidden_pageid'] ) && $_POST['wtgcsv_hidden_pageid'] == 'data' && isset($_POST['wtgcsv_hidden_panel_name']) && $_POST['wtgcsv_hidden_panel_name'] == 'exportsingletabledata'){
            ...
            if($wtgcsv_is_free && !strstr( $_POST['wtgpt_exporttable'] , 'wtgcsv_' )){
                wtgcsv_notice('This action is not permitted for security, please purchase the full edition to export any table from the Wordpress database in this way.','warning','Large','Export Not Permitted','','echo');
            }else{
...
function wtgcsv_get_template_bypostrequest(){
    ...
    if(isset($_POST["wtgcsv_opencontentdesign"]) && isset($_POST["wtgcsv_templatename_and_id"])){
        ...
        $post_id = wtgcsv_extract_value_from_string_between_two_values('(',')',$_POST["wtgcsv_templatename_and_id"]);    
        
        // get post (post type: wtgcsvtemplate)
        $template_post_object = get_post($post_id);
        ...
        // get requested design id from $_POST
        if(isset($_POST['wtgcsv_templatedesign_menu_all'])){
            $templatedesign_array['template_id'] = $_POST["wtgcsv_templatedesign_menu_all"];    
        }elseif(isset($_POST['wtgcsv_templatedesign_menu_project'])){
            $templatedesign_array['template_id'] = $_POST["wtgcsv_templatedesign_menu_project"];    
        }
    }
...
/**
* Updates project meta description template with the giving value
*/
function wtgcsv_update_project_metadescription($metadescription,$project_code){
    $project_array = wtgcsv_get_project_array($project_code);
    if(!$project_array){
        return false;
    }else{
        $project_array['metadescription_template'] = $metadescription;
    }
...
function wtgcsv_get_project_datecolumn($project_code){
    ...
    if(isset($project_array['date_column']['table_name']) && isset($project_array['date_column']['column_name'])){
        return $project_array['date_column']['table_name'] . '-' . $project_array['date_column']['column_name'];
    }else{
