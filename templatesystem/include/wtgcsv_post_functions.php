...
function wtgcsv_create_posts_advanced($project_code,$posts_target,$request_method){
    ...
    // get all title template designs linked to project and put them in array - post_id as key
        //$default_title_template = wtgcsv_get_titletemplate($project_array['default_titletemplate_id']);
    
    // get all content template designs linked to project and put them in array - post_id as key
        // function to be created to return the required array
...
function wtgcsv_create_posts_basic($project_code,$request_method){ 
    ...
    foreach( $records as $record_array ){
        ...
        if(isset($project_array['custom_fields']['basic'])){ 
            ...
            echo '<pre>';
            var_dump($project_array['custom_fields']['basic']);
            echo '</pre>';
            ...
            foreach($project_array['custom_fields']['basic'] as $key => $cfrule){
                ...
                add_post_meta($my_post['ID'], $cfrule['meta_key'], $record_array[$cfrule['column_name']]);     
            }
        }

        // TODO: HIGHPRIORITY, this will be included in free edition, create thumbnail/featured image attachment if activated ( wordpress post image )
            // eci_isthumbnailneeded( $csv,$recordarray,$my_post['ID'] );                                       
