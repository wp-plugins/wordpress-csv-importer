...
$panel_array['panel_title'] = __('Default Content Template (Currently: '.wtgcsv_get_default_contenttemplate_name().')');// user seen panel header text 
...
<?php wtgcsv_panel_header( $panel_array );?>
    ...
    <form id="wtgcsv_form_opentemplate_id" action="<?php echo $wtgcsv_form_action;?>" method="post" name="wtgcsv_form_opentemplate_name">
        ...
        <input type="hidden" name="wtgcsv_change_default_template" value="true">        
    
        <h4>Current Project Designs</h4>
        <?php wtgcsv_display_projectdesigns_buttonlist('wtgcsv_selecttemplate_fromproject_id');?>          
    
        <h4>All Designs</h4>
        <?php wtgcsv_display_alldesigns_buttonlist();?>            
...
$panel_array['panel_number'] = $panel_number;// number of panels counted on page, used to create object ID
$panel_array['panel_title'] = __('Open Templates');// user seen panel header text 
$panel_array['pageid'] = $pageid;// store the $pageid for sake of ease
$panel_array['tabnumber'] = $wtgcsv_tab_number; 
$panel_array['panel_id'] = $panel_array['panel_name'].$panel_number;// creates a unique id, may change from version to version but within a version it should be unique
...
$panel_array['panel_number'] = $panel_number;// number of panels counted on page, used to create object ID
$panel_array['panel_title'] = __('Template Editor *global panel*');// user seen panel header text 
$panel_array['pageid'] = $pageid;// store the $pageid for sake of ease
$panel_array['tabnumber'] = $wtgcsv_tab_number; 
$panel_array['panel_id'] = $panel_array['panel_name'].$panel_number;// creates a unique id, may change from version to version but within a version it should be unique
...
<?php wtgcsv_panel_header( $panel_array );?>
    ...
    <h2>Create/Edit Your Template Design Below</h2>           
    <?php wtgcsv_display_designtype_menu($templatedesign_array['template_id']);//TODO:change this too an ID in $_POST?>
...
$panel_array['panel_number'] = $panel_number;// number of panels counted on page, used to create object ID
$panel_array['panel_title'] = __('Dynamic Content Design Rules (by value)');// user seen panel header text 
$panel_array['pageid'] = $pageid;// store the $pageid for sake of ease
$panel_array['tabnumber'] = $wtgcsv_tab_number; 
$panel_array['panel_id'] = $panel_array['panel_name'].$panel_number;// creates a unique id, may change from version to version but within a version it should be unique
...
<?php wtgcsv_panel_header( $panel_array );?>
    ...
    <p>        
        Table Column: 
        <select name="wtgcsv_dynamiccontentdesignrules_select_columnandtable" id="wtgcsv_dynamiccontentdesignrules_select_columnandtable_formid" style="width: 500px; display: none; ">
    ...
    <p>                    
        Design: 
        <select name="wtgcsv_dynamiccontentdesignrules_select_templateid" id="wtgcsv_dynamiccontentdesignrules_select_template_formid">
