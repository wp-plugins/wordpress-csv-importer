<?php      
global $wtgcsv_display_testing_info,$wtgcsv_mpt_arr,$wpdb,$wtgcsv_options_array,$wtgcsv_nav_type,$wtgcsv_projectslist_array,$wtgcsv_schedule_array;
...
if($wtgcsv_nav_type == 'css'){ 
    ...
    $wtgcsv_form_action = wtgcsv_link_toadmin('wtgcsv_yourcreation','&tabnumber=' . $tab_number);
    ...
    foreach($wtgcsv_mpt_arr[$pageid]['tabs'] as $tab=>$values){
        ...
        if($wtgcsv_mpt_arr[ $pageid ]['tabs'][ $counttabs ]['display'] == true){
            ...
            include(WTG_CSV_DIR.'pages/'.$pagefolder.'/wtgcsv_tab'.$counttabs.'_pagecreation.php');
            echo '</div>';
