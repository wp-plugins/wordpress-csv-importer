<?php 
global $wtgcsv_display_testing_info,$wtgcsv_mpt_arr,$wpdb,$wtgcsv_options_array,$wtgcsv_nav_type;
...
wtgcsv_notice('Most of the More screen will be under construction until 2013 as some of the features still require Web Services (SOAP) to be fully developed and tested.','warning','Large');
...
if($wtgcsv_nav_type == 'css'){ 
    ...
    $wtgcsv_form_action = wtgcsv_link_toadmin('wtgcsv_yourmore','&tabnumber=' . $tab_number);
    ...
    foreach($wtgcsv_mpt_arr[$pageid]['tabs'] as $tab=>$values){
        ...
        if($wtgcsv_mpt_arr[ $pageid ]['tabs'][ $counttabs ]['display'] == true){
            ...
            include(WTG_CSV_DIR.'pages/'.$pagefolder.'/wtgcsv_tab'.$counttabs.'_more.php');
            echo '</div>';
